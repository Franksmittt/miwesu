...
export async function POST(request: Request) {
  ...
  const { data, error } = await resend.emails.send({
    from: `MIWESU Test <${fromEmail}>`,
    to: [to],
    subject: 'MIWESU – Test email from admin',
    html: '<p>If you received this, Resend is working from your app. Sent at ' + new Date().toISOString() + '</p>',
  })
